...
photo_path = "F:\\Photos\\iPhone8\\Downloads\\2020"
os.chdir(photo_path)
...
for root, dirs, files in os.walk(os.getcwd()):
    ...
    for name in files:
        if "video_" in name or "Video_" in name or "video" in name or "Video" in name:
            continue
        #删除文件名带Edited或者已编辑字样的文件
        if "Edited" in name or "已编辑" in name:
        ...
        for num in range(1,20):
            ...
            for ext in extensions: 
                if "_"+num+ext in name "Edited" in name or "已编辑" in name:
                    print(name)
                    os.remove(os.path.join(root, name))
